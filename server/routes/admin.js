...
router.post("/send-message", async (req, res) => {
  try {
    const { userId, phone, message, selectedType } = req.body;
    console.log(req.body);

    if (!userId || !message || !selectedType || !phone) {
      return res.status(400).json({ message: "Required fields: userId, message, selectedType, phone" });
    }

    // 🔍 Find match involving the recipient
    ...
    if (!match) {
      return res.status(404).json({ message: "Active match not found for this user" });
    }
...
router.get('/delete-notifications', async (req,res) => {
  const notificationToDelete = req.params.id

  try {
    // Fetch users from Auth0
    const deletedNotification = await Notifications.deleteOne({notificationToDelete});
    // Send response
...
router.get('/response-rate', async (req, res) => {
  try {
    ...
    const groupedMessages = await getGroupedMessages();
